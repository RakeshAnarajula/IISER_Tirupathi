...
export function ChemistryPage() {
  ...
  const data = researchPagesData["chemistry"];
  const accent = data?.accentColor || '#d97706';

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    document.title = "Department of Chemistry | IISER Tirupati";
  }, []);
    ...
    <div className="research-page-wrapper">
      ...
      <div className="site-container-wide research-body-container">
        <div className="research-body-grid">
          <ResearchSidebar activePath="/chemistry" />
          ...
          <main className="research-main-content">
            ...
            {activeTab === 'areas' && (
              <div className="research-areas-section animate-fade">
                ...
                <div style={{ marginTop: '32px', padding: '28px', borderRadius: '18px', background: 'linear-gradient(135deg, #451a03 0%, #78350f 100%)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px', boxShadow: '0 10px 30px rgba(69,26,3,0.2)' }}>
                  <div>
                    <span style={{ fontSize: '0.74rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: '#fde68a', fontWeight: 800 }}>Flagship R&amp;D Initiative</span>
                    <h4 style={{ margin: '6px 0 8px', fontSize: '1.25rem', color: '#fff' }}>Catalysis for Net-Zero Carbon Economy</h4>
                  ...
                  <Link 
                    ...
                    style={{ background: '#f59e0b', color: '#451a03', padding: '12px 22px', borderRadius: '10px', fontWeight: 700, fontSize: '0.86rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px' }}
                  >
                    <span>Instrumentation Facility</span>
