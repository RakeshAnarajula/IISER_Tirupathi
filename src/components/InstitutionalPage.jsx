...
function OfficerAvatar({ name, image }) {
  ...
  const src = image || `https://ui-avatars.com/api/?name=${encodeURIComponent(cleanName)}&background=0F3E6D&color=FFFFFF&bold=true&size=128`;
...
export function InstitutionalPage({ pageKey, onNavigate, onBack }) {
  ...
  const siblings = Object.entries(aboutAndPeopleData)
    .filter(([key, data]) => data.category === page.category)
